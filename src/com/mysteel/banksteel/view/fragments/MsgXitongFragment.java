...
import com.mysteel.banksteel.view.adapters.MsgXitongAdapter;
 ...
 */
public class MsgXitongFragment extends BaseFragment implements OnClickListener,IListViewInterface,IXListViewListener, ISystemManagerView
{
		...
		public boolean toggle = false; //是否显示选择按钮
	    ...
	    {
	    	currentView = inflater.inflate(R.layout.fragment_xitong_msg, container,
					false);
	    ...
	    {
	    	msgAdapter = new MsgXitongAdapter(getActivity());
		...
		private void getMsgAdapter(int position)
		{
	    	boolean isCheck = datas.get(position).getSelected();
	    	if(isCheck){
		...
		private void delMsg(){
			...
			if(!TextUtils.isEmpty(resultID)){
				...
				//Tools.showToast(getActivity(), resultID);
...
//			    	ll_del_layout.setVisibility(View.GONE);
//			    	ll_nullmsg.setVisibility(View.VISIBLE);
//			    	EventBus.getDefault().post(true, "msgxitongfragment_del");
					page = "1";
					getMessage();
			    }else{
			    	msgAdapter.reSetListView(datas, this.toggle);
