 ...
 * 
 */
public class SpeechFindGoodsActivity extends BaseActivity implements
		OnClickListener, OnCompletionListener, IBuyView
{
...
//					ivRelisten.setImageResource(R.drawable.voice_01);
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		setContentView(R.layout.activity_speech_findgoods);
		initViews();
	}
	...
	{
		super.initViews();
		tvTitle.setText("语音找货");
		tvRightText.setText("去吐槽");

		progressBar = (ProgressBar) findViewById(R.id.pb_progressbar);
		etDemand = (EditText) findViewById(R.id.et_speech_demand);
...
//		tvUseVoice = (TextView) findViewById(R.id.tv_speech_usevoice);
		etPhone = (EditText) findViewById(R.id.et_speech_phonenumber);
		btnFindGood = (Button) findViewById(R.id.btn_speech_findgoods);
		speakingPopus = (LinearLayout) findViewById(R.id.ll_speaking_popup);
		rlRelisten = (RelativeLayout) findViewById(R.id.rl_speech_relisten);
//		ivRelisten = (ImageView) findViewById(R.id.iv_speech_relisten);
		ivDelRelisten = (ImageView) findViewById(R.id.iv_speech_del);
		showTime = (Chronometer) findViewById(R.id.timedown);
		...
		// rlRelisten.setOnLongClickListener(new OnLongClickListener()
		// {
		// @Override
		...
		rlUseVoice.setOnTouchListener(new OnTouchListener()
		{
			@Override
				...
				} else if (event.getAction() == MotionEvent.ACTION_UP)
				{
					...
					if (timeStart != 0)// timestart=0,说明未开始录音
					{
						...
						if (recoredTime < 3)
						{
							delFile(path);
							rlRelisten.setVisibility(View.GONE);
							Tools.showToast(SpeechFindGoodsActivity.this,
									"录音时间不足3秒");
	...
	{
		this.timeLeft = total;
		showTime.setOnChronometerTickListener(new OnChronometerTickListener()
		{
			@Override
			public void onChronometerTick(Chronometer chronometer)
			{
				...
				{
					// 剩余时间<0,录音结束
					Tools.showToast(SpeechFindGoodsActivity.this, "录音时间到");
					showTime.stop();
					// 录音停止
					stop();
					// 隐藏 speakingPopus
...
//				.setTextColor(getResources().getColor(R.color.font_white_one));
	...
	private Runnable startRunnable = new Runnable()
	{
		...
		public void run()
		{
			...
			showTime.start();
			mSensor.start(getApplicationContext(), voiceName);// 开始测声录音
	...
	private void soundUse()
	{
		...
		if (file.exists())
		{
			...
			// 点击声音文件播放声音
			rlRelisten.setOnClickListener(new OnClickListener()
			{
				@Override
	...
	private void createFileName()
	{
		...
		voiceName = SharePreferenceUtil.getString(this,
...
//			break;
	...
	private void publishFindGoods()
	{
		...
		{
			Tools.showToast(this, "请检查网络是否连接");
			return;
		}
		...
		{
			/** 语音找货 */
			String url = RequestUrl.getInstance(this).getUrl_getVoicePublish(
		...
		} else
		{
			...
			if (content.length() < 5)
			{
				...
				{
					Tools.showToast(this, "请录音找货,或者填写找货需求");
					return;
				}
				Tools.showToast(this, "你提供的资料太少");
				return;
			}

			String url = RequestUrl.getInstance(this).getUrl_getPublishFastBuy(
	...
	public void testUpload(String url)
	{
		...
		http.send(HttpRequest.HttpMethod.POST, url, params,
				new RequestCallBack<String>()
				{
					...
					{
						Tools.showToast(SpeechFindGoodsActivity.this, "上传失败！");
						btnFindGood.setEnabled(true);
					}
				});
	}
	...
	{
		Tools.showToast(this, "快捷找货发布成功,我们的业务员稍后会和您联系");
		finish();
	}
