 ...
 */
public class SellIntentionDetailActivity extends SwipeBackActivity implements View.OnClickListener, IOrderTradeView
{
    ...
    //卖家姓名,卖家公司，买家姓名，买家公司，卖家（本人）电话
    private TextView tv_sell_name, tv_sell_company, tv_buy_name, tv_buy_company, tv_address/*, tv_sell_phone*/;
    private ImageView img_yixiang_phone, img_yixiang_message;//买家电话，买家聊天入口
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.activity_sell_intenttion);
    ...
    public void onClick(View v)
    {
        ...
        switch (v.getId())
        {
            case R.id.menu_layout:// 返回
                finish();
                break;
            case R.id.img_yixiang_phone://买家联系电话
                if ((data != null && data.getData() != null && data.getData().getOrder() != null) && !TextUtils.isEmpty(data.getData().getOrder().getBuyPhone()))
                {
            ...
            case R.id.btn_speech_findgoods:
                ...
                if (data != null && data.getData() != null && data.getData().getOrder() != null)
                {
                    ...
                    {
                        if ("2".equals(data.getData().getOrder().getStatus()) && !TextUtils.isEmpty(data.getData().getOrder().getAppraiseStatus())
                                && ("0".equals(data.getData().getOrder().getAppraiseStatus()) || "1".equals(data.getData().getOrder().getAppraiseStatus())))
                        {
                            Intent i1 = new Intent(this, BuyAssessActivity.class);
                            i1.putExtra("orderId", data.getData().getOrder().getId());
                            i1.putExtra("TAG", "1");
                            startActivity(i1);
                        }
                    }
...
//        tv_sell_phone = (TextView) findViewById(R.id.tv_sell_phone);// 卖家本人的电话
        tv_address = (TextView) findViewById(R.id.tv_address);//卖家自己的地址信息
        ...
        buy_circle.setOnClickListener(this);
    ...
    private void changeView()
    {
        if (data != null && data.getData() != null && data.getData().getOrder() != null)
        {
            ...
            {
                btn_speech_findgoods.setText("待上传凭证");
...
//                btn_speech_findgoods.setBackgroundResource(R.drawable.findfoodup);
            ...
            {
                if ("0".equals(data.getData().getOrder().getAppraiseStatus()) || "1".equals(data.getData().getOrder().getAppraiseStatus()))
                {
                    imRightImage.setVisibility(View.VISIBLE);
                    btn_speech_findgoods.setText("发表评价");
                    btn_speech_findgoods.setBackgroundResource(R.drawable.findfood);
                } else
                {
                    imRightImage.setVisibility(View.VISIBLE);
            ...
            tv_buy_company.setText(data.getData().getOrder().getBuyMemberName());
//            tv_sell_phone.setText(data.getData().getOrder().getQuotPhone());
            ...
            {
                Tools.loadImage(data.getData().getPartnerUserPhoto(), buy_circle);//买家头像
            }
            String photo = SharePreferenceUtil.getString(this,
                    Constants.USER_PHTHO);
            if(!TextUtils.isEmpty(photo))
            {
                Tools.loadImage(photo, sell_circle);//卖家头像
            }

        }
    }

    @Subscriber(tag = "sellfinish")
